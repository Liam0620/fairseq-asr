...
class W2lDecoder(object):
    ...
    def generate(self, models, sample, prefix_tokens=None):
        ...
        #end = time.clock()
        #print('emissions Running time: %s Seconds'%(end-start))

        #result = self.decode(emissions)
        #end2 = time.clock()
        #print('decode Running time: %s Seconds' % (end2 - end))
        #sys.exit()
    ...
    def get_emissions(self, models, encoder_input):
        ...
        #print(11111,encoder_input['source'].size(),encoder_input['padding_mask'].size())
        #sys.exit()
        encoder_out = models[0](**encoder_input)
        if self.criterion_type == CriterionType.CTC:
            if "encoder_out_final" in encoder_out:
                emissions = models[0].get_normalized_probs2(encoder_out, log_probs=True) #+ models[0].get_normalized_probs(encoder_out, log_probs=True)
            else:
