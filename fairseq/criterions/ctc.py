...
class CtcCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if 'encoder_out_final' in net_output:
            #print('encoder_out_final',net_output['encoder_out_final'].size())
            lprobs2 = model.get_normalized_probs2(
                net_output, log_probs=True
            ).contiguous()  # (T, B, C) from the encoder
            #print('encoder_out_final_lpobs',lprobs2.size())
            #sys.exit()
            use_lprobs2 = True
        else:
            use_lprobs2 = False
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        if w_total > 0:
            metrics.log_derived(
                "wer",
                lambda meters: round(meters["_w_errors"].sum.item() * 100.0 / meters["_w_total"].sum.item(), 3)
                if meters["_w_total"].sum > 0
                else float("nan"),
            )
            metrics.log_derived(
                "raw_wer",
                lambda meters: round(meters["_wv_errors"].sum.item() * 100.0 / meters["_w_total"].sum.item(), 3)
                if meters["_w_total"].sum > 0
                else float("nan"),
            )
