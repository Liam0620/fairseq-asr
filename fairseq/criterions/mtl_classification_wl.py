...
@register_criterion('mtl_classification_wl')
class MTL_ClassifyCriterion_wl(FairseqCriterion):
    ...
    def __init__(self, task, wer_args=None, zero_infinity=None,sentence_avg=None,class_num=None):
        ...
        '''
        self.criterion_sid = Uncertainty_weighed_loss_AM(AM_softmax(768, num_classes, margin=0.2, scale=30))
        self.criterion_lid = Uncertainty_weighed_loss_AM(AM_softmax(768, num_classes, margin=0.2, scale=30))
        #self.criterion_vad = nn.NLLLoss(reduction='sum')
    ...
    def extract_logits(self,model,sample):
        ...
        logists = self.criterion_lid.criterion.extract_logits(feats)
        return logists
...
class Uncertainty_weighed_loss_AM(nn.Module):
    def __init__(self, criterion):
        super(Uncertainty_weighed_loss_AM, self).__init__()
        self.log_var = torch.nn.Parameter(torch.zeros(1).cuda(), requires_grad=True)
        self.weight = torch.exp(-self.log_var)
        self.criterion = criterion
    def forward(self, x, label):
        loss,costh_m_s = self.criterion(x,label)
        loss = loss * self.weight + self.log_var
        return loss,costh_m_s
