...
class MTL_TS_VAD_only(FairseqCriterion):
    ...
    def __init__(self, task, sentence_avg=None,class_num=None):
        ...
        #self.criterion_ts_vad =  nn.CrossEntropyLoss(weight=torch.FloatTensor([2,1,1]))
        self.criterion_ts_vad_wpl = WPL_loss(weights=[1,1,0.1])
    ...
    def forward(self, model, sample, reduce=True):
        ...
        # source_len = sample['net_input']['source'].size(1)
        ...
        if ori_targets.size(1) == 1:
            ...
            for i in range(len(targets)):
                targets[i, 0:input_lengths[i].cpu().numpy()] = ori_targets[i]
        else:
            targets = ori_targets
        ...
        if vad:
            ...
            loss = self.criterion_ts_vad_wpl(logits_vad.view(-1, logits_vad.size(-1)), targets)
            #print(111,loss,loss2)
            #sys.exit()
