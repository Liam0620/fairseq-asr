...
class Wav2Vec2Model(BaseFairseqModel):
    ...
    def apply_mask(self, x, padding_mask):
        ...
        if self.mask_channel_prob > 0:
            ...
            x[mask_channel_indices] = 0 ##0 modified by mli
    ...
    def forward(self, source, padding_mask=None, mask=True, features_only=False,vad=False,stage=None):
        if stage=='asr':
            assert vad and features_only
            x, x_vad = self.encoder.forward_vad(source, padding_mask=padding_mask, stage=stage)
            return {"x": x, "x_vad": x_vad, "padding_mask": padding_mask}

        #print('raw_data',source.size())
        ...
        if self.post_extract_proj is not None:
            ...
            #print('post_extract_proj',features.size())
        ...
        if mask:
            ...
            y = unmasked_features
            mask_indices = None
        #print('transformer_input',x.size())
        #if vad:
        #    x_vad = x
        #    if stage=='vad':
        #        return {"x": None,"x_vad": x_vad, "padding_mask": padding_mask}

        #x = self.encoder(x, padding_mask=padding_mask)

        if vad:
            x,x_vad = self.encoder.forward_vad(x, padding_mask=padding_mask,stage=stage)
        else:
            x = self.encoder(x, padding_mask=padding_mask)
        #print('transformer_output',x.size())


        if features_only:
            if vad:
                if stage=='cnn_vad_asr':
                    return {"x": x, "x_vad": cnn_features, "padding_mask": padding_mask}
                else:
                    return {"x": x,"x_vad": x_vad, "padding_mask": padding_mask}
            return {"x": x, "padding_mask": padding_mask}

        if self.quantizer:
            #print('quantizer_input(mask positions)',y.size())
            q = self.quantizer(y, produce_targets=False)
            #print('quantizer_output',q["x"].size())
            y = q["x"]
            num_vars = q["num_vars"]
            code_ppl = q["code_perplexity"]
            prob_ppl = q["prob_perplexity"]
        ...
        #print('x_mask_ids_transformer_out',x.size())
        if self.target_glu:
            y = self.target_glu(y)
            negs = self.target_glu(negs)

        x = self.final_proj(x)
        #print('final_x',x.size())
    ...
    def extract_features(self, source, padding_mask, mask=False,vad=False,stage=None):
        res = self.forward(source, padding_mask, mask=mask, features_only=True,vad=vad,stage=stage)
        if vad:
...
class TransformerEncoder(nn.Module):
    def __init__(self, args):
        ...
        # modified by mli
        '''
        self.vad_layer = TransformerSentenceEncoderLayer(
    ...
    def extract_features(self, x, padding_mask=None):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.layerdrop):
                ...
                #sys.exit()
        #end = time.clock()
        #print('transformer Running time: %s Seconds' % (end - start))

        if torch.any(torch.isnan(x)):
            print('wav2vec_encoder feats333333 has nan',x.size()) # by mli

        # T x B x C -> B x T x C
        x = x.transpose(0, 1)
        return x

    def forward_vad(self, x, padding_mask=None,stage=None):
        if not stage=='asr':
            x,x_vad = self.extract_features_vad(x, padding_mask,stage=stage)
        else:
            x,x_vad = self.extract_features_vad_asr(x, padding_mask)

        if self.layer_norm_first:
            if stage is None:
                x = self.layer_norm(x)
                x_vad = self.layer_norm(x_vad)
            elif stage=='vad':
                x_vad = self.layer_norm(x_vad)
            elif stage=='asr':
                x = self.layer_norm(x)
    ...
    def extract_features_vad(self, x, padding_mask=None,stage=None):
        x_conv = self.pos_conv(x.transpose(1, 2))
        x_conv = x_conv.transpose(1, 2)
        x += x_conv
        x_vad = x
        ...
        x_vad = x_vad.transpose(0, 1)

        return x,x_vad

    def extract_features_vad_asr(self, x, padding_mask=None):
...
class TransformerSentenceEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.layer_norm_first:
            ...
            #print(43254234)
            if torch.any(torch.isnan(x)):
                print('encoder layer1111111 has nan', x.size())  # by mli

            x_tmp, attn_tmp = self.self_attn(
                query=x,
                key=x,
                value=x,
                key_padding_mask=self_attn_padding_mask,
            ...
            #print(3213123,x_tmp.size())
            if torch.any(torch.isnan(x_tmp)): # by mli
                x, attn = self.self_attn(
                    query=x,
                    key=x,
                    value=x,
