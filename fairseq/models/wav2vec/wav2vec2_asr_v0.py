...
from fairseq.modules import Fp32GroupNorm,Fp32LayerNorm,GradMultiply,GumbelVectorQuantizer,LayerNorm,MultiheadAttention, SamePad,TransposeLast
...
class Wav2VecCtc2(BaseFairseqModel):
    ...
    def build_model(cls, args, task):
        """Build a new model instance."""
        base_architecture(args)
        w2v_encoder = Wav2VecEncoder2(args, task.target_dictionary)
        return cls(w2v_encoder, args)
    ...
    def get_normalized_probs2(self, net_output, log_probs):
        """Get normalized probabilities (or log probs) from a net's output."""

        logits = net_output["encoder_out_final"]
        if log_probs:
            return utils.log_softmax(logits.float(), dim=-1)
        else:
            return utils.softmax(logits.float(), dim=-1)
...
class Wav2VecEncoder2(FairseqEncoder):
    def __init__(self, args, tgt_dict=None):
        ...
        if tgt_dict is not None:
            self.proj = Linear(d, len(tgt_dict))
            self.proj2 = Linear(d, len(tgt_dict))
            self.re_proj = Linear(len(tgt_dict),d)
        else:
            self.proj = None
            self.proj2 = None
            self.re_proj = None

        self.final_encoder_layer = TransformerSentenceEncoderLayer(
                    embedding_dim=w2v_args.encoder_embed_dim,
                    ffn_embedding_dim=w2v_args.encoder_ffn_embed_dim,
                    num_attention_heads=w2v_args.encoder_attention_heads,
                    dropout=w2v_args.dropout,
                    attention_dropout=w2v_args.attention_dropout,
                    activation_dropout=w2v_args.activation_dropout,
                    activation_fn=w2v_args.activation_fn,
                    layer_norm_first=w2v_args.layer_norm_first,
                    )
    ...
    def forward(self, source, padding_mask, tbc=True, **kwargs):
        ...
        x_fe = self.final_dropout(x)
        if self.proj:
            x = self.proj(x_fe)
            x_emb = self.re_proj(x)

            contextual_x,_ = self.final_encoder_layer(x_emb)

            final_x = contextual_x + x_fe
            final_x = self.proj2(final_x)

        return {
            "encoder_out": x,  # T x B x C
            "encoder_out_final": final_x,  # T x B x C
            "encoder_padding_mask": padding_mask,  # B x T
            "padding_mask": padding_mask,
        }
...
@register_model_architecture("wav2vec_ctc2", "wav2vec_ctc2")
def base_architecture(args):
    args.no_pretrained_weights = getattr(args, "no_pretrained_weights", False)
    args.dropout_input = getattr(args, "dropout_input", 0)
    args.final_dropout = getattr(args, "final_dropout", 0)
