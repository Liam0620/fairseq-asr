...
@register_model("wav2vec_class_TS_vad")
        ...
        if not self.training:
            ...
            with torch.no_grad():
                if stage=='cnn_only':
                    ...
                    x_seq = torch.cat((x_seq, ts_embedding), dim=-1)
                    x_seq = self.pre_scd_layer(x_seq)
                    x_seq = self.scd_encoder(x_seq)  # ,_
            ...
            ts_embedding = input_embeddings.expand(input_embeddings.size(0),x_seq.size(1),input_embeddings.size(-1))
            x_seq = torch.cat((x_seq,ts_embedding),dim=-1)
            #print(333333, ts_embedding.size())
            x_seq = self.pre_scd_layer(x_seq)
            #print(111,x_seq.size())
            x_seq = self.scd_encoder(x_seq) #,_
            #print(2222,x_seq.size())
            x_seq = self.post_scd_layer(x_seq)
            #print(3333, x_seq.size())
            if self.proj:
                x_seq = self.proj(x_seq)
                #print(444444, x_seq.size())
            #sys.exit()
            return {
                "encoder_seq_out": x_seq,  # B x T x C
                "encoder_padding_mask": padding_mask,  # B x T
...
@register_model_architecture("wav2vec_class_TS_vad", "wav2vec_class_TS_vad")
